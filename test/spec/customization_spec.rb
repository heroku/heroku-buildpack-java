...
describe "Heroku's Java Buildpack" do
  context "with the MAVEN_CUSTOM_GOALS environment variable set" do
    it "will only use goals from MAVEN_CUSTOM_GOALS" do
      new_default_hatchet_runner("test/fixtures/simple-http-service") do |app|
        app.before_deploy do
          app.set_config({:MAVEN_CUSTOM_GOALS => "site"})
        end
    ...
    #      expect(app.output).to include("./mvnw -DskipTests clean")
    #      expect(app.output).to include("[INFO] --- maven-clean-plugin:3.1.0:clean (default-clean) @ simple-http-service ---")
    #
  ...
  #        expect(app.output).to include("./mvnw -X clean install")
